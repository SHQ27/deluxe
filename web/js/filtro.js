var filtro = {
	...
	methods : {
		filterSlide : function() {
			var filterFirstPosition = $('#wrapper-content').position().top + $('.wrapper #content').position().top + $('.filtro').position().top + 20;
			
			
			
		    $(window).scroll(function() {
		        if(filterFirstPosition >= $(window).scrollTop()) {
		            if($('.filtro .content').hasClass('fixed')) {
		                $('.filtro .content').removeClass('fixed');
		                $('.filtro .orderBy').show();
		                
		            }
		        } else { 
		            if(!$('.filtro .content').hasClass('fixed')) {
		                $('.filtro .content').addClass('fixed');
		                $('.filtro .orderBy').hide();
		            }
		        }
		    });
	...
	setUp: function( rangeMin, rangeMax, rangeSettedMin, rangeSettedMax ) {
		...
		buttons.click(function( ev ){
			...
			obj.methods.select( container, currentList, activedList );
			
            // Habilito el custom scroll
			activedList = container.find('.active');
			$( activedList ).height( 300 );
            $( activedList ).mCustomScrollbar();
