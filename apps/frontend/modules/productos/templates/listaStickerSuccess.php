<?php sfContext::getInstance()->getResponse()->setTitle( sfContext::getInstance()->getResponse()->getTitle() . ' - ' . $productoSticker->getDenominacion() . ' de ' . $productoGenero->getDenominacion() ); ?>
    
<?php sfContext::getInstance()->getResponse()->addMeta('description', 'Comprá ' . $productoSticker->getDenominacion() . ' de ' . $productoGenero->getDenominacion() .' en DeluxeBuys. La Tienda Online de Moda e Indumentaria hasta un 70% OFF' ); ?>

<?php sfContext::getInstance()->getResponse()->addMeta('keywords', $productoSticker->getDenominacion() . ', ' . $productoGenero->getDenominacion()  . ', Indumentaria Femenina, Indumentaria Masculina, Ropa, Tienda Online, DeluxeBuys' ); ?>

<div class="listadoProductos listaTags">

    <h1 class="fleft"><?php echo $productoSticker->getDenominacion(); ?> para <?php echo $productoGenero->getDenominacion(); ?></h1>
    ...
    <?php 
    	include_component(
    		...
    		'filter', 
    		array(
    			'filters'  => array('idProductoSticker' => $productoSticker->getIdProductoSticker(), 'idProductoGenero' => $productoGenero->getIdProductoGenero(), 'rango' => $rango),    		        
    			'query'    => $query
    		)
    	);
    ?>
