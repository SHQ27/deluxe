...
{	
	this.init = function()
	{
	    ...
	    $(".sf_admin_batch_actions_choice .submit").click( function(){
	        if ( $(".sf_admin_batch_actions_choice .batch_action").val() == 'DEVOLVER' )
            {
	            var ids = [];
	            ...
	            window.location = '/backend/devueltosMarcas/devolver?ids=' + ids.join(',');
            }
	    } );
...
$(document).ready( function() { new devueltosMarcas(); } );